...
*** FUNCIONES PRESENTE EN EL CÓDIGO ***
app.js
...
Esta función se ejecuta al enviar el formulario con nuestra búsqueda, permitiendo pulsar ENTER tras escribir un título, mejorando la usuabilidad y permitiendo realizar nuestra búsqueda sin necesidad de pulsar el botón. Es bastante similar a la anterior.
Esta función se ejecuta cuando se envía el formulario de búsqueda. Previene el comportamiento por defecto del formulario, captura el título de la película, construye la 
...
Esta es una de las funciones más importantes, tras recibir los datos de la película procedentes de la API de OMDB los muestra rederizados en la página. Cuando nuestra petición tiene éxito [data.Response === "True"] se crean tarjetas con la información de las películas que además muestran un botón que permite, si se quiere, guardar la película en la lista de favoritas.

En caso de que la película no se encuentra se muestra el sencillo mensaje de “Película no encontrada”.

5. guardarPelicula = (title, year, director, poster) => { ... }:
Gracias a esta función podemos contar con nuestra colección de película. Al hacer click en el botón la información de la película es guardada en el almacenamiento local (localStorage). Además la función puede verificar si existe le peli, en cuyo caso no la añade para evitar duplicados y muestra un mensaje de alerta. Si se guarda con éxito muestra también un mensaje de alerta indicando que ha tenido éxito. 
...
API de OMDB: Externo, para obtener info de películas.
...
2. Guardado de Películas:
   - Al mostrar una película existe un botón “guardar” para añadir a favoritos.
